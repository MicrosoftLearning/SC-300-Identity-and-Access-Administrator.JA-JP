...
ms.openlocfilehash: e4b865c11bfa8948bd1039bc07eaa3eac954c5c8
ms.sourcegitcommit: b5fc07c53b5663eaa1883cf38b70c57cd88470ca
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 06/29/2022
ms.locfileid: "146741400"
---
...
# <a name="roberts----probably-need-to-swap-the--for-a--then-select-for-screen-read-purposes"></a>RobertS --> おそらく、画面の読み取り目的で、> を ", then select" に入れ替える必要があります。
...
# <a name="roberts----task-1-step-6-and-task-2-step-3--do-we-want-to-use-those-passwords--with-the-hacks-that-have-occurred-in-the-past-i-think-we-were-going-with-enter-a-secure-password-that-you-will-remember--i-know-you-copied-this-directly-out-of-the-existing-labs-and-that-the-lab-will-be-hosted-in-an-unknown-tenant--so-this-is-an-ask-for-your-opinion-not-an-ask-to-change-at-this-point"></a>RobertS --> タスク 1、手順 6、タスク 2、手順 3 - これらのパスワードを使用しますか?  過去に発生したハッキングで、私たちは "あなたが記憶する安全なパスワードを入力します" としていたと思います。  これを既存のラボから直接コピーし、ラボが不明なテナントでホストされることを知っています。  これはあなたの意見を求めるもので、この時点で変更するように求めるものではありません。
...
# <a name="roberts----i-think-you-need-to-close-the-new-application-page-to-get-back-to-the-enterprise-apps-menu-before-the-next-step"></a>RobertS --> 次の手順の前に、[エンタープライズ アプリ] メニューに戻るには、[新しいアプリケーション] ページを閉じる必要があると思います。
...
1. 設定が完了したら、変更を保存してアップロードしてユーザーを追加します。

1. ファイルが正常にアップロードされたことが通知されます。  [送信] を選択してユーザーを追加します。 
...
1. 以前に使用していない場合は、Azure AD PowerShell モジュールを追加する必要があります。  Install-Module AzureAD コマンドを実行します。  メッセージが表示されたら、Y キーを押して続行します。
...
1. すべての新しいユーザーに共通の一時パスワードを割り当てるには、次のコマンドを実行し、TempPW をユーザーに提供するパスワードに置き換えます。  
...
1. これで、新しいユーザーの作成に取りかかることができます。  次のコマンドにユーザー情報が入力され、実行されます。  追加するユーザーが複数ある場合は、メモ帳の txt ファイルを使用してユーザー情報を追加し、PowerShell にコピーして貼り付けることができます。 
...
3. [ユーザー] ページで、 **[Raul Razo]** を選択します。

4. 左側のナビゲーションで、**[ライセンス]** を選択します。

5. **[+ 割り当て]** ボタンを選択します。 

6. [ライセンス割り当ての更新] ページで、**Windows 10/11 Enterprise E3** ライセンスのチェック ボックスをオンにします。

    ![[ライセンス割り当ての更新] ページとライセンス オプションが強調表示されている画面イメージ](./media/lp1-mod2-assign-user-license-options.png)

7. 完了したら、**[保存]** を選択します。

8. 画面の上部にある **[ホーム]** を選択し、 **[Contoso]** を選択し、 **[ユーザー]** を選択し、 **[Raul Razo]** を選択します。

10. ライセンスが割り当てられていることに注目してください。
