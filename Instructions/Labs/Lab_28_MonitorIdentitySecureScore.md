...
1. **[セキュリティ | ID セキュリティ スコア]** で、提供された情報を確認します。

1. **[ID セキュリティ スコア]** の値や **[スコア履歴]** などの情報に注目してください。

1. 下にスクロールして、**[推奨事項]** を表示します。
...
1. ID セキュリティ ポスチャの 1 つの領域を改善するには、**[ユーザー リスク ポリシーですべてのユーザーを保護する]** を選択します。

1. 表示されるページで、リスクを確認します。 保護されていないユーザーは 33 人になります。 さらに、脅威を解決する方法に関する**アクション プラン**が表示されます。

1. リンク "**次の手順に従って、条件付きアクセス ポリシーを最初から、またはテンプレートを使用して作成します**" を選択します。 記事の手順を確認します。

1. 記事のタブを閉じ、**Microsoft Entra ID** を開いた状態でタブに戻ります。

1. 左側のメニューで **[条件付きアクセス]** を選択します。

1. **[新しいポリシーの作成]** を選択します。

1. 次の値を使用してポリシーを作成します。

| フィールド | 値 |
| :--- | :--- |
| 名前 | **ユーザー リスク保護ポリシー** |
| 代入 | 1. **[ユーザーまたはエージェント (プレビュー) が選択されていない]** を選択します |
|             | 2.**[含める]** タブで **[すべてのユーザー]** をマークします |
|             | 3. **[除外]** タブで、**[ユーザーとグループ]** を使用して **[MOD 管理者]** を除外します |
| ターゲット リソース | **[ターゲット リソースが選択されていない]** を選択し、**[すべてのリソース (以前の [すべてのクラウド アプリ])]** を選択します |
| ネットワーク | 既定値のままにします |
| 条件 | 1.**[0 個の条件が選択されました]** を選択します |
|            | 2. **[ユーザー リスク]** で、**[未構成]** リンクを選択します。 |
...
1. **[ポリシーの有効化]** を **[レポート専用]** に設定します。  WhatIf 関数を使用して、ポリシーを確認してから有効にできます。

1. **［作成］** を選択します

### 演習の概要

この演習では、ID セキュリティ スコア ダッシュボードを確認し、サインイン リスクの条件付きアクセス ポリシーの下書きを作成して改善のための処置を開始しました。 この演習では、セキュリティ スコアによって ID のセキュリティ体制が改善されるしくみについて説明しました。
