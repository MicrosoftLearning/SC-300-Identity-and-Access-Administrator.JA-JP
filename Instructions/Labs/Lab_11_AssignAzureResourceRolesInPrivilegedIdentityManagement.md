...
lab:
  ...
  description: Privileged Identity Management (PIM) を使用して、Azure リソースへの特権アクセスを割り当てます。
  duration: 10 minutes
  level: 300
  islab: true
  primarytopics:
...
1. 「**Microsoft Entra Privileged Identity Management**」を検索して選択します。

1. [Privileged Identity Management] ページの左側のナビゲーションで **[Azure リソース]** を選択します。
...
1. [サブスクリプション] ドロップダウンで、MOC Subscription##### という項目を選択します。 次に、画面の下部にある **[リソースの管理]** を選択します。

1. [Azure リソース - 検出] ページで、サブスクリプションを選択します。
...
1. [割り当ての追加] ページで、 **[ロールの選択]** メニューを選択し、 **[API Management サービス共同作成者]** を選択します。

1. **[メンバーの選択]** で **[メンバーが選択されていません]** を選択します。

1. [メンバーまたはグループの選択] で、ロールが割り当てられる組織の管理者ロール **User1-######@LODSPRODMCA.onmicrosoft.com** を検索します。 **[Select]\(選択\)** をクリックします。
...
>**注:** このラボ環境にはセキュリティが適用されているため、以下の手順を完了できません。  ユーザー インターフェイスの手順を確認してください。ただし、変更を適用することはできません。  この問題に対する回避策の整備に鋭意取り組んでいるところです。
...
1. 管理するサブスクリプションを選択して、その概要ページを開きます。

1. **[管理]** で、 **[割り当て]** を選択します。

1. **[資格のある割り当て]** タブの [アクション] 列で、使用できるオプションを確認します。
...
この演習では、Privileged Identity Management を使用して適格な Azure リソース ロールを割り当て、必要に応じてそれをアクティブにし、割り当てを削除しました。 この演習では、Just-In-Time アクセスにより、Azure リソースに対する永続的な特権を削減する方法を示しました。
