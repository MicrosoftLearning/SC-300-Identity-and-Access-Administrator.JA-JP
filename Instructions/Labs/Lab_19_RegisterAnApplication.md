...
#### <a name="task-5---add-a-scope"></a>タスク 5 - WEB API を登録し、スコープを追加する

1. **[アプリの登録]** ページで、**[+ 新規登録]** を選択します。

1. **[アプリケーションの登録]** ブレードで既定値を使用して、"**Demo API**" という名前のアプリを登録します。 

3. **リダイレクト URI の追加** と **プラットフォーム設定の構成** に関するセクションはスキップします。 ユーザーは対話的にログインしないため、Web API のリダイレクト URI を構成する必要はありません。
