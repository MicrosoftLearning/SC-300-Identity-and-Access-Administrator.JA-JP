...
5. 右上の「×」をクリックして、プラットフォームの構成を閉じます。
...
#### <a name="task-6---add-a-scope-requiring-admin-consent"></a>タスク 7 - Demo App に Demo API へのスコープを許可する

1. **アプリの登録** で、**Demo APP** を選択します。

1. **APIのアクセス許可** で、「＋アクセス許可の追加」をクリックします。

1. 「**自分のAPI**」タブから「Demo API」を選択し、前のステップで作成した２つのスコープをチェックします。

1. **アクセス許可の追加** をクリックして、Demo App にスコープへのアクセスを許可します。
