...
1. **[管理]** タブで、[Microsoft Entra ID] セクションの **[Microsoft Entra ID でログイン]** のチェック ボックスをオンにします。
...
1. VM が開き、すべてのソフトウェアが読み込まれるのを待ちます。
...
1. Windows はログインを確認し、通常の画面に開く必要があります。
...
1. **<<server name>>-EntraID.RDP** ファイルをもう一度起動します。

1. AdeleV や AlexW、DiegoS などの他の Microsoft Entra ユーザーとしてログインしてみてください。
