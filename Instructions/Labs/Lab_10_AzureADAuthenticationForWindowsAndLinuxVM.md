...
1. 仮想マシンの設定時に作成した管理者 (vmEntraAdmin) のユーザー名とパスワードを使用します。
...
1. 他の Microsoft Entra ID メンバーとしてログインしてみてください。
