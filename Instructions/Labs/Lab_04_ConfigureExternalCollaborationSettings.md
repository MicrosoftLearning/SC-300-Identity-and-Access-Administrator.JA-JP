...
1. **[すべてのユーザー]** メニュー項目を開き、**[ユーザー設定]** を選択します。

1. **[外部ユーザー コラボレーションの設定の管理]** を選択します。

1. **[ユーザー フローによるゲスト セルフサービスのサインアップを有効にする]** 設定に対して、 **[はい]** がマークされていることを確かめます。

1. ページの最上部で **[保存]** を選択します。

#### タスク 2 - 外部コラボレーションの設定を構成する

1. 左側のナビゲーション メニューで、**[Entra ID]** の下にある **[外部 ID]** を選択し、**[すべての ID プロバイダー]** を選択します。

1. プロバイダーの一覧で **[メールのワンタイム パスコード]** 項目を選んだあと、**[構成済み]** を選択します。

    >**注** - ワンタイム パスコードは、ユーザーを招待して組織に参加させるための非常に安全な方法です。
    
1. **[はい]** が選択されていることを確認します。

1. 必要に応じて、 **[保存]** を選択します。

1. **[External Identities]** メニューに戻ります。

1. 左側の **[外部コラボレーション設定]** を選択します

1.  **[ゲスト ユーザー アクセス]** で、使用可能なアクセス レベルを確認し、 **[ゲスト ユーザーのアクセスを、自分のディレクトリ オブジェクトのプロパティとメンバーシップに制限する (最も厳しい制限)]** を選択します。
...
1.  **[ゲストの招待設定]** で、 **[メンバー アクセス許可を持つゲストを含むメンバー ユーザーと特定の管理者ロールに割り当てられたユーザーがゲスト ユーザーを招待できる]** を選択します。
...
1.  **[コラボレーションの制限]** で、使用可能なオプションを確認し、既定の設定をそのまま使用します。
...
この演習では、ゲストのセルフサービス サインアップを有効にし、ゲストのアクセス レベル、招待のアクセス許可、コラボレーション制限などの外部コラボレーション設定を構成しました。 この演習では、テナント レベルでゲスト アクセスを管理する方法を示しました。
